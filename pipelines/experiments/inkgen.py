...
class InkGen(AMLPipelineHelper):
    ...
    def build(self, config):
        ...
        InkGen_component = self.component_load("InkGen")
        ...
        def demo_pipeline_function(data_path):
            ...
            InkGen_component_step = InkGen_component(
                ...
                cell_size = config.inputs.cell_size,
                optimizer = config.inputs.optimizer,
                learning_rate = config.inputs.learning_rate,
                warmup_steps = config.inputs.warmup_steps,
                decay_rate = config.inputs.decay_rate,
                num_clusters = config.inputs.num_clusters,
                K = config.inputs.K,
                z_size = config.inputs.z_size,
                clip_value = config.inputs.clip_value,
                lstm_clip_value = config.inputs.lstm_clip_value,
                style_equalization = config.inputs.style_equalization
                )
 
            env_variables = {"AZUREML_COMMON_RUNTIME_USE_SBOM_CAPABILITY": "True"}
            self.apply_recommended_runsettings(
                "InkGen",
                InkGen_component_step,
                gpu=True,
    ...
    def pipeline_instance(self, pipeline_function, config):
        ...
        data_path = self.dataset_load(
            name=config.inputs.data_dir,
            version=config.inputs.data_dir_version,
        )

        # we simply call the pipeline function
        demo_pipeline = pipeline_function(data_path)
