...
def generate_conditionally(text, save_name, device, cell_size=400, num_clusters=20, K=10, z_size=0, random_state=700, bias=0., bias2=0., char_to_code_file='data/char_to_code.pt', state_dict_file='trained_models/conditional_epoch_60.pt', priming_x = None, priming_text = '', x_r=None, model_type = None):
...
def generate_teacherforcing(data, onehots, save_name, device, cell_size=400,num_clusters=20,K=10,z_size=0,random_state=700,bias=0.,bias2=0.,char_to_code_file='data/char_to_code.pt',state_dict_file='trained_models/conditional_epoch_60.pt', priming_x = None, priming_text = '', x_r=None, model_type = None):
    np.random.seed(random_state)
    char_to_code = torch.load(char_to_code_file, weights_only=True)
    if model_type == 'alexrnn':
        model = LSTMSynthesis(len(char_to_code), cell_size, num_clusters, K, z_size=z_size)
    ...
    model.load_state_dict(
        torch.load(state_dict_file, map_location=torch.device('cpu'))['model']
    )
    model = model.to(device)
    data = data[:,:-1]
    B,T,_ = data.shape
