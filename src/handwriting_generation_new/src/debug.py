...
from generate_train_data import generate_teacherforcing
    ...
    for epoch in range(start, args.num_epochs):
        ...
        for i, (data, masks, onehots) in enumerate(train_loader):
            ...
            if args.style_equalization:
                ...
                outputs, _ = model(x, onehots, w, kappa, state1, state2, state3, type='training')
            ...
            save_data = {
                "end": end.detach().tolist(),
                "stop": stop.detach().tolist(),
                "weights": weights.detach().tolist(),
                "mu_1": mu_1.detach().tolist(),
                "mu_2": mu_2.detach().tolist(),
                "log_sigma_1": log_sigma_1.detach().tolist(),
                "log_sigma_2": log_sigma_2.detach().tolist(),
                "rho": rho.detach().tolist()
            }
            import pdb
            pdb.set_trace()
            with open("../repo_results/teacher_forcing/train_output.json", "w") as f:
                json.dump(save_data, f, indent=4)
            # debug teacher forcing
            generate_teacherforcing(model,
                                    x,
                                    onehots,
                                    device,
                                    args.cell_size,
                                    args.K,
                                    save_path="../repo_results/teacher_forcing",
                                    random_state=700,
                                    bias=0.,
                                    bias2=0.,
                                    x_r=None)
...
def main():
    ...
    if enable_dist:
        ...
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, shuffle=False)#, drop_last=True)
