...
class TransformerDecoderLayers(nn.Module):
    def __init__(self, d_model, nhead=2, dim_feedforward=2048, num_layers=2):
        super().__init__()
...
class TransformerSynthesis(nn.Module):
    ...
    def __init__(self, vocab_len, cell_size, num_clusters, K, z_size=0, use_posenc=True):
        ...
        d_model = cell_size + (z_size if z_size>0 else 0)

        # 用一个线性层投影到 d_model
        self.transformer_proj = nn.Linear(in_dim, d_model)

        # 位置编码
        self.pos_encoding = PositionalEncoding(d_model) if use_posenc else None

        # Decoder
        self.transformer = TransformerDecoderLayers(
            d_model = d_model,
            nhead = 2,
        ...
        self.linear = nn.Linear(cell_size + d_model, 2 + num_clusters*6)
    ...
    def forward(self, 
        ...
        # 投影到 (B, seq_len, d_model)
        transformer_in = self.transformer_proj(cat_in)
        # 位置编码
        if self.pos_encoding is not None:
            transformer_in = self.pos_encoding(transformer_in, t, type)
        if type=='training':
        # 经过 Transformer Decoder
            ...
            transformer_out = self.transformer(tgt=transformer_in, memory=None,tgt_mask=tgt_mask)  # => (B, seq_len, d_model)
        elif type=='inference':
            transformer_ins.append(transformer_in)
            transformer_in = torch.cat(transformer_ins,dim=1)
            seq_len = len(transformer_ins)
            tgt_mask = generate_subsequent_mask(seq_len, device=transformer_in.device)
            # transformer_in = torch.cat([transformer_in,previous_transformer_in], dim=1)
            transformer_out = self.transformer(tgt=transformer_in, memory=None, tgt_mask=tgt_mask)

        # =========== MDN 输出层 ============
        # 拼接 h1s (B, seq_len, cell_size) 和 transformer_out (B, seq_len, d_model)
        if type=='training':
            final_cat = torch.cat([h1s, transformer_out], dim=-1)  # => (B, seq_len, cell_size + d_model)
        elif type=='inference':
            if transformer_out.shape[1]==1:
                final_cat = torch.cat([h1s, transformer_out], dim=-1)
            else:
                final_cat = torch.cat([h1s, transformer_out[:,-1,:].unsqueeze(1)], dim=-1)
        ...
        # 数值检查（可选）
        for n, a in zip(['ws','h1s','w','kappa','phi','params'],
                        [ws,     h1s,   w,   kappa,  phi,   params]):
